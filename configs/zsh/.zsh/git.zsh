...
alias gbr='git for-each-ref --color=always --sort="-committerdate:iso8601" --format="%(color:green)%(committerdate:relative)%(color:reset)|%(color:blue)%(committername)%(color:reset)|%(refname:short)" refs/remotes/ | column -s "|" -t | less -R'
